...
const AdminTeachers: React.FC = () => {
    ...
    useEffect(() => {
        const fetchTeachers = async () => {
            try {
                setLoading(true);
                const [teachersData, groupsData, sessionsData] = await Promise.all([
                    api.getTeachers(),
                    api.getGroups(),
                    api.getMySessions(),
                ]);
    ...
    const getTeacherStats = (teacherId: string) => {
        const groupsCount = groups.filter((g) => g.teacherId === teacherId).length;
        const oneToOneClasses = new Set(
            sessions
                .filter((session) => session.type === 'ONE_TO_ONE' && session.teacherId === teacherId)
                .map((session) => session.classId),
        );
        return {
            groups: groupsCount,
            oneToOnes: oneToOneClasses.size,
            rating: null,
        };
    };
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="p-4 flex items-center">
                                                <img
                                                    src={teacher.avatarUrl || `https://ui-avatars.com/api/?name=${teacher.name.replace(' ', '+')}`}
                                                    alt={teacher.name}
                                            ...
                                            <td className="p-4 text-text-secondary italic">{stats.rating ?? 'N/A'}</td>
                                            <td className="p-4">
